...
@section('contenido')
  ...
  @endif
    ...
    <div class="modal-dialog">
      ...
      <button type="submit" form="nuevaObservacionForm" class="btn btn-primary" id="guardarObservacion">Guardar</button>
      </div>
      </div>
    </div>
    </div>
  ...
  @endforeach
    ...
    document.addEventListener('DOMContentLoaded', function () {
        // Botón para eliminar una observación
        const botonesEliminarObservacion = document.querySelectorAll('.eliminar-observacion');
        botonesEliminarObservacion.forEach(boton => {
                    ...
                    if (result.isConfirmed) {
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                Swal.fire(
                                    'Error',
                                    'No se pudo eliminar la observación: ' + (data.message || 'Error desconocido'),
                                    'error'
                                );
                            }
                        })
        ...
        // Botón para agregar una nueva observación
        const btnGuardarObservacion = document.getElementById('guardarObservacion');
        if (btnGuardarObservacion) {
            btnGuardarObservacion.addEventListener('click', function () {
                const descripcion = document.getElementById('descripcionObservacion').value.trim();

                if (!descripcion) {
                    Swal.fire(
                        'Advertencia',
                        'Por favor, ingrese una descripción para la observación.',
                        'warning'
                    );
                    return;
                }
                ...
                fetch('{{ route("observaciones.store") }}', {
                    method: 'POST',
                    body: formData,
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest'
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        Swal.fire(
                            '¡Guardado!',
                            'La observación ha sido guardada correctamente.',
                            'success'
                        ).then(() => {
                        ...
                        Swal.fire(
                            'Error',
                            'No se pudo guardar la observación: ' + (data.message || 'Error desconocido'),
                            'error'
                        );
                    }
                })
